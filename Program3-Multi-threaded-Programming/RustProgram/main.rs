...
fn main() {
    ...
    let mut threads1: Vec<thread::JoinHandle<usize>> = Vec::new();

    for i in xs {
        let thr = thread::spawn(move || map_data(&i));
        threads1.push(thr);
    }
    ...
    let mut threads2: Vec<thread::JoinHandle<usize>> = Vec::new();
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>> {
    ...
    for i in 0..num_partitions {
        ...
        if (i == (num_partitions - 1)) && (odd == true) {
            x1.push(v[counter]);
        }
