...
{
	int listenSocketFD, establishedConnectionFD, portNumber, fileLength, charsRead, charsSent, totalSent, totalRead, charDiff, currIndex;
	...
	while (1) {
		...
		if (connections < 5) {
			...
			if (spawnPid == 1) { fprintf(stderr, "dec_server.c: There was an error on fork #: %d", i); exit(1); }
			else if (spawnPid == 0) {
				...
				if (strcmp(clientName, "dec_client") == 0) {
					...
					// Get the cipher text message from the client and store it.
					...
					// decrypt ciphertext message using key, minus newline
					for (i = 0; i < fileLength - 1; i++) {
						...
						// subtract key and cipher char, perform modulus operation
						charDiff = (completeMsg[i] - completeKey[i]);

						if (charDiff < 0) {
							charDiff = charDiff + 27;
						}

						convertedMsg[i] = charDiff;
